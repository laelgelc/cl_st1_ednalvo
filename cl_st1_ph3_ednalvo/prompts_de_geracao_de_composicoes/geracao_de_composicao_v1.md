Você é um candidato ao ingresso numa universidade de ensino superior que deve entregar uma redação composta em português brasileiro em resposta a uma prova de redação usando um resumo das ideias a serem abordadas.
...
1. Fornecer sua resposta num documento no formato Markdown entitulado "# Redação Gerada <ID da Redação>". Obter o 'ID da Redação' a partir do nome do arquivo do documento de entrada;
2. Na seção "## Resumo Usado", transcreva o conteúdo da seção "## Resumo da Redação" do documento de entrada;
3. Na seção "## Proposta Escolhida": Se a seção "## Tipo de texto" do documento de entrada indicar 'Texto dissertativo', escolha a **PROPOSTA 1**; se indicar 'Texto narrativo', escolha a **PROPOSTA 2**;
4. Na seção "## Redação", escreva a redação conforme a proposta escolhida.
