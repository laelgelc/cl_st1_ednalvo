...
Annotate composition files with linguistic tags using an LLM.

For each supported text/Markdown file found recursively inside --input-dir, this
programme sends the following to the selected LLM:

1. The annotation prompt supplied with --prompt
2. The tagset supplied with --tagset
3. The composition text

Input policy:

- .txt files:
  The whole file is submitted to the LLM.

- .md and .markdown files:
  Only the contents under the level-2 Markdown heading "## Redação" are submitted
  to the LLM. Content before "## Redação" and content under later level-2 headings
  is ignored.

Output policy:

- The output directory/subdirectory structure mirrors the input structure.
- All output files use the same stem as the input file, but the extension is
  always .txt.

For example:

    corpus/01_composicoes/maiores_notas/32757.txt

becomes:

    corpus/03_composicoes_anotadas/maiores_notas/32757.txt

and:

    corpus/01_composicoes/menores_notas_gemini/32610_composicao_gemini-3.6-flash.md

becomes:

    corpus/03_composicoes_anotadas/menores_notas_gemini/32610_composicao_gemini-3.6-flash.txt
...
PROGRAMME_NAME = "anote_composicoes.py"
...
def natural_sort_key(value: str) -> list[Any]:
    """Return a natural-sort key for filenames or paths containing numbers."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Annotate composition files recursively using an LLM."
    )

    parser.add_argument("--input-dir", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--model", required=True, default=DEFAULT_MODEL)
    parser.add_argument("--prompt", required=True, type=Path)
    parser.add_argument("--tagset", required=True, type=Path)
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Start processing at this filename after recursive natural sorting. "
            "The value is matched against file names, not full relative paths."
        ),
    )
    parser.add_argument(
        "--reprocess",
        action="store_true",
        help="Reprocess files even if the corresponding output .txt already exists.",
    )
...
def make_config(args: argparse.Namespace) -> Config:
    ...
    log_file = (
        ...
        if args.log_file
        else output_dir / "anote_composicoes.log"
    )
    manifest_file = (
        resolve_path(args.manifest_file, script_dir)
        if args.manifest_file
        else output_dir / "anote_composicoes_manifest.json"
    )
    ...
    return Config(
        ...
        tagset=resolve_path(args.tagset, script_dir),
...
def validate_config(config: Config) -> None:
    ...
    if not config.tagset.exists() or not config.tagset.is_file():
        raise ConfigurationError(
            f"Tagset file is missing or unreadable: {relpath(config.tagset, config.script_dir)}"
        )
...
def load_required_text(path: Path, label: str) -> str:
    """Load and validate a required UTF-8 text document."""
...
def extract_markdown_section(markdown_text: str, heading: str) -> str:
    """
    Extract the contents under a level-2 Markdown heading.

    For example, with heading='Redação', this extracts the text after:

        ## Redação

    until the next level-2 heading, or until the end of the file.

    The heading must be written as a level-2 Markdown heading. These forms are
    accepted:
    ...
    if not match:
        raise ValueError(f"Markdown section not found: ## {heading}")
    ...
    if next_match:
        section_text = markdown_text[section_start:next_match.start()]
    else:
        section_text = markdown_text[section_start:]

    section_text = section_text.strip()
    if not section_text:
        raise ValueError(f"Markdown section is empty: ## {heading}")

    return section_text


def read_composition_text(path: Path) -> str:
    """
    Read the composition text to be submitted to the LLM.

    - .txt files:
      Use the entire file.

    - .md/.markdown files:
      Use only the contents under '## Redação'.
    """
...
def discover_input_files(config: Config) -> list[Path]:
    """Discover eligible input files recursively in natural relative-path order."""
    files = [
        path
    ...
    files.sort(
        key=lambda item: natural_sort_key(
            str(item.relative_to(config.input_dir)).replace("\\", "/")
        )
    )
    ...
    if config.start_filename:
        ...
        if config.start_filename not in filenames:
            raise ConfigurationError(
                f"--start-filename not found in input tree: {config.start_filename}"
            )
        files = files[filenames.index(config.start_filename) :]
...
def output_path_for_input(config: Config, input_file: Path) -> Path:
    """
    Return the mirrored .txt output path for an input composition.

    The relative directory path under --input-dir is preserved under --output-dir.
    The output suffix is always .txt, regardless of whether the input was .txt,
    .md or .markdown.
    """
    relative_path = input_file.relative_to(config.input_dir)
    return (config.output_dir / relative_path).with_suffix(".txt")
...
def build_llm_request(
    ...
    """Combine prompt, tagset and source composition into one stateless LLM request."""
    return (
        f"{prompt_text}\n\n"
        f"---\n\n"
        f"## Metadados da tarefa\n\n"
        f"- Tarefa: Anotação linguística de composição\n"
        f"- Ficheiro de entrada: {input_file.name}\n"
        f"- Tipo de ficheiro de entrada: {input_file.suffix.lower()}\n\n"
        f"---\n\n"
        f"## Tagset\n\n"
        f"{tagset_text.strip()}\n\n"
        f"---\n\n"
        f"## Composição de entrada\n\n"
        f"{input_text.strip()}"
    )
...
def process_one_file(
    ...
    relative_input_file = input_file.relative_to(config.input_dir)

    item: dict[str, Any] = {
        "input_filename": input_file.name,
        "input_file": relpath(input_file, config.script_dir),
        "relative_input_file": str(relative_input_file),
        ...
        "tagset_file": relpath(config.tagset, config.script_dir),
    ...
    try:
        if output_file.exists() and not config.reprocess:
            ...
            logging.info(
                ...
                relpath(input_file, config.input_dir),
                relpath(output_file, config.output_dir),
            )
            return item

        input_text = read_composition_text(input_file)
        if not input_text.strip():
            raise ValueError(f"Input composition is empty: {relative_input_file}")

        item["input_sha256"] = sha256_text(input_text)
        item["submitted_text_characters"] = len(input_text)

        if input_file.suffix.lower() in {".md", ".markdown"}:
            item["input_extraction_policy"] = "markdown_section_##_Redacao"
        else:
            item["input_extraction_policy"] = "full_text_file"

        request_text = build_llm_request(
            prompt_text=prompt_text,
            tagset_text=tagset_text,
        ...
        logging.error("FAILED %s: %s", relpath(input_file, config.input_dir), exc)
        logging.debug(
            "Traceback for %s:\n%s",
            relpath(input_file, config.input_dir),
...
def build_manifest(
    ...
    return {
        ...
        "paths": {
            ...
            "tagset": relpath(config.tagset, config.script_dir),
            "env_file": relpath(config.env_file, config.script_dir),
        ...
        "tagset": {
            "file": relpath(config.tagset, config.script_dir),
            "sha256": tagset_sha256,
            "characters": len(tagset_text),
        },
        "processing": {
            ...
            "order": "recursive_natural_relative_path_order",
            "supported_input_suffixes": sorted(SUPPORTED_INPUT_SUFFIXES),
            "markdown_input_policy": "extract_contents_under_level_2_heading_Redacao",
            "text_input_policy": "use_entire_file",
            "output_layout": "mirrors_input_directory_structure",
            "output_filename_policy": "same_input_stem_with_txt_suffix",
        },
        "counts": {
...
def main() -> int:
    """Run the batch annotation workflow."""
    ...
    logging.info("Prompt: %s", relpath(config.prompt, config.script_dir))
    logging.info("Tagset: %s", relpath(config.tagset, config.script_dir))
    ...
    try:
        ...
        prompt_text = load_required_text(config.prompt, "Prompt")
        tagset_text = load_required_text(config.tagset, "Tagset")
        prompt_sha256 = sha256_text(prompt_text)
        tagset_sha256 = sha256_text(tagset_text)
        ...
        if config.workers == 1:
            for index, input_file in enumerate(planned_files):
                logging.info("Processing file: %s", relpath(input_file, config.input_dir))
                results.append(
                    process_one_file(
                        input_file,
                        config=config,
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=config.workers) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_file):
                    ...
                    try:
                        ...
                        logging.error("Unexpected worker failure for %s: %s", input_file.name, exc)
                        results.append(
                            {
                                "input_filename": input_file.name,
                                "input_file": relpath(input_file, config.script_dir),
                                "relative_input_file": str(input_file.relative_to(config.input_dir)),
                                "input_suffix": input_file.suffix.lower(),
                                "output_file": relpath(
                                    output_path_for_input(config, input_file),
                                    config.script_dir,
                                ),
                                "model": config.model,
                                "status": "failed",
