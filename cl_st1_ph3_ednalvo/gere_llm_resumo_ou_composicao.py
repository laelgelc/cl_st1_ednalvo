#!/usr/bin/env python3
"""
gere_llm_resumo_ou_composicao.py

Generate LLM summaries or compositions from a directory of source documents.

The programme supports two commands:

1. resuma
   For each text/Markdown file in --input-dir, send the prompt document specified
   by --prompt plus the input file contents to the selected LLM. The response is
   written to --output-dir using the same input filename stem plus "_resumo".

2. componha
   For each text/Markdown file in --input-dir, send the prompt document specified
   by --prompt plus the input file contents to the selected LLM. The response is
   written to --output-dir using the same input filename stem with "_resumo"
   replaced by "_composicao_<model>". If the input filename does not contain
   "_resumo", the suffix is appended.
...
Examples:
    ...
    python gere_llm_resumo_ou_composicao.py resuma \\
        --input-dir corpus/01_composicoes/maiores_notas \\
    ...
    python gere_llm_resumo_ou_composicao.py componha \\
        ...
        --output-dir corpus/03_composicoes_llm \\
...
PROGRAMME_NAME = "gere_llm_resumo_ou_composicao.py"
PROGRAMME_VERSION = "v1"

COMMAND_RESUMA = "resuma"
COMMAND_COMPONHA = "componha"
...
def natural_sort_key(value: str) -> list[Any]:
    """Return a natural-sort key for filenames containing numbers."""
...
def model_family(model: str) -> str:
    ...
    # Conservative default requested by the examples: gpt-* OpenAI, gemini-* Gemini.
    # For unknown names, OpenAI is the more likely text-generation default in this project.
    return "openai"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Generate LLM summaries or compositions for text files."
    )

    subparsers = parser.add_subparsers(dest="command", required=True)

    for command_name in (COMMAND_RESUMA, COMMAND_COMPONHA):
        command_parser = subparsers.add_parser(
            command_name,
            help=(
                "Summarize compositions"
                if command_name == COMMAND_RESUMA
                else "Generate compositions from summaries"
            ),
        )
...
def make_config(args: argparse.Namespace) -> Config:
    ...
    log_file = (
        ...
        if args.log_file
        else output_dir / "gere_llm_resumo_ou_composicao.log"
    )
    manifest_file = (
        resolve_path(args.manifest_file, script_dir)
        if args.manifest_file
        else output_dir / "gere_llm_resumo_ou_composicao_manifest.json"
    )
...
def validate_config(config: Config) -> None:
    ...
    if config.command not in {COMMAND_RESUMA, COMMAND_COMPONHA}:
...
def discover_input_files(config: Config) -> list[Path]:
    """Discover eligible input files in natural filename order."""
    files = [
        path
    ...
    if config.start_filename:
        ...
        if config.start_filename not in filenames:
            raise ConfigurationError(
                f"--start-filename not found in input directory: {config.start_filename}"
            )
...
def output_path_for_input(config: Config, input_file: Path) -> Path:
    """Return the output Markdown path for a given input file and command."""
    model_name = safe_model_name_for_filename(config.model)

    if config.command == COMMAND_RESUMA:
        output_stem = f"{input_file.stem}_resumo"
    elif config.command == COMMAND_COMPONHA:
        if "_resumo" in input_file.stem:
            output_stem = input_file.stem.replace("_resumo", f"_composicao_{model_name}")
        else:
            output_stem = f"{input_file.stem}_composicao_{model_name}"
    else:
        raise ConfigurationError(f"Unsupported command: {config.command}")

    return config.output_dir / f"{output_stem}.md"


def build_llm_request(command: str, prompt_text: str, input_file: Path, input_text: str) -> str:
    """Combine prompt and source document into one stateless LLM request."""
    if command == COMMAND_RESUMA:
        task_label = "Resumo de composição"
        document_label = "Composição de entrada"
    elif command == COMMAND_COMPONHA:
        task_label = "Geração de composição"
        document_label = "Documento de entrada"
    else:
...
def process_one_file(
    ...
    try:
        if output_file.exists() and not config.reprocess:
            ...
            logging.info("SKIPPED_EXISTING %s -> %s", input_file.name, output_file.name)
            return item
        ...
        logging.info("SUCCESS %s -> %s", input_file.name, output_file.name)
        return item
        ...
        logging.error("FAILED %s: %s", input_file.name, exc)
        logging.debug("Traceback for %s:\n%s", input_file.name, traceback.format_exc())
        return item
...
def main() -> int:
    ...
    try:
        ...
        logging.info(
            "Discovered %s supported input files; planned %s.",
            discovered_count,
            len(planned_files),
        )
        ...
        if config.workers == 1:
            for index, input_file in enumerate(planned_files):
                logging.info("Processing file: %s", input_file.name)
                results.append(
                    process_one_file(
                        input_file,
                        config=config,
